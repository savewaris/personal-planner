...
export const HabitTracker: React.FC<{ isModalOpenExternal?: boolean; onCloseModalExternal?: () => void }> = ({
  ...
  return (
    <div className="glass-card p-6 border border-white/10 rounded-2xl space-y-6">
      ...
      {/* Side-by-Side Body: 6-Month Heatmap (Left, 7 cols) & Daily Checklist (Right, 5 cols) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Side: 6-Month Heatmap */}
        <div className="lg:col-span-7">
          <HabitHeatmap logs={allLogs} />
        </div>

        {/* Right Side: Daily Checklist */}
            ...
            <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
              {habits.map((habit) => (
                <motion.div
                  key={habit.id}
                  whileHover={{ x: 3 }}
                  className="p-3 rounded-xl bg-white/5 border border-white/5 hover:border-white/15 flex items-center justify-between gap-3 transition-all group"
                >
                  {/* Left: Checkbox + Name */}
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <span className={`text-xs font-semibold truncate ${habit.completedToday ? "line-through text-zinc-500" : "text-zinc-100"}`}>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <FlameStreakBadge streak={habit.streak || 0} />
