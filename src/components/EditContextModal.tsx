...
interface EditContextModalProps {
  isOpen: boolean;
  context: { id: string; name: string; color?: string | null };
  onClose: () => void;
...
export const EditContextModal: React.FC<EditContextModalProps> = ({
  ...
  const [selectedColor, setSelectedColor] = useState<ContextColor>(
    (context.color as ContextColor) || "blue"
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(null);
      await onUpdate(context.id, { name: trimmed, color: selectedColor });
      onClose();
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              name="contextName"
              data-testid="edit-context-name-input"
              value={name}
              onChange={(e) => {
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-neutral-800">
            {onDelete ? (
              <button
                ...
                disabled={isDeleting}
                className="px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-lg transition-colors cursor-pointer"
              >
